...
namespace UpdateProgram
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            if (param != "")
            {
                double version = double.Parse(param);
                double newVersion = GetNewVersion();

                if (version < newVersion)
                {
                    DialogResult result = MessageBox.Show("发现新版本，是否更新？", "AiTools更新提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                ...
                else
                {
                    if (args.Length > 1)
                    {
                        MessageBox.Show(new Form(), "当前版本为最新版本！", "更新提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
        }
